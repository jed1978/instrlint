...
describe('scanProject', () => {
  ...
  describe('clean-project', () => {
    it('detects claude-code via CLAUDE.md root file (low confidence)', () => {
      const result = scanProject(CLEAN_PROJECT);
      expect(result.tool).toBe('claude-code');
      // clean-project has no .claude/ dir, so confidence is low
  ...
  describe('error handling', () => {
    it('throws when projectRoot does not exist', () => {
      expect(() =>
        scanProject('/this/path/does/not/exist/anywhere')
