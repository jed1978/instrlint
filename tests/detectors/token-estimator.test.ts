...
describe("estimateFallback", () => {
  ...
  it("fallback is within ±30% of tiktoken for English text", () => {
    const text =
      "Always use TypeScript strict mode. Never commit secrets. Use pnpm for package management.";
    ...
    if (measured.method === "measured") {
      ...
      expect(ratio).toBeLessThan(1.5); // char-based fallback can over-estimate by up to ~50%
    }
