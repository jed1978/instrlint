...
export async function runAll(
  ...
  if (opts.fix) {
    ...
    if (total === 0) {
      ...
      if (deadFixed > 0)
        output.log(`  ${chalk.yellow('⚠')}  ${t('fix.removedDeadRules', { count: String(deadFixed), s: plural(deadFixed) })}`);
      if (staleFixed > 0)
        output.log(`  ${chalk.yellow('⚠')}  ${t('fix.removedStaleRefs', { count: String(staleFixed), s: plural(staleFixed) })}`);
      if (dupeFixed > 0)
        output.log(`  ${chalk.yellow('⚠')}  ${t('fix.removedDuplicates', { count: String(dupeFixed), s: plural(dupeFixed) })}`);
      output.log(chalk.gray('  ─'.repeat(30)));
      output.log(chalk.green(`  ${t('status.fixedIssues', { count: String(total), s: plural(total) })}`));
      output.log('');
    }
    return { exitCode: 0 };
  }
