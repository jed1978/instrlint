...
- **Single unified skill source.** `skills/instrlint/SKILL.md` is the canonical skill file used by all install targets (Claude Code, Codex). It follows the [Agent Skills spec](https://agentskills.io/specification) — directory name matches `name` field. `install-command.ts` always reads from this path regardless of target platform.
