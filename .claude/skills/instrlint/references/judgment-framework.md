...
- **confirmed**: This rule is genuinely better enforced by a git hook or path-scoped rule file; keeping it in CLAUDE.md is redundant
...
## CLAUDE.md Refactoring — 4-Bucket Framework
...
5. **Execute the chosen actions**: bucket 1 → extract to `.claude/rules/` with `paths:` / `globs:` frontmatter; bucket 2 → extract without path-scope frontmatter; bucket 3 → replace with a one-line pointer
...
| **2. Extractable, no savings** | Global, loaded in nearly every conversation (> 80% load rate) | Extract to shorten CLAUDE.md — total tokens unchanged |
| **3. Delete, not move** | Duplicates source code (e.g. type definitions already in `src/types.ts`) | Replace with a one-line pointer |
| **4. Stay in CLAUDE.md** | Cross-conversation decisions / commands / state | Don't move |
...
| Architecture decisions | 4 — Stay in CLAUDE.md | Cross-file design principles needed every conversation |
